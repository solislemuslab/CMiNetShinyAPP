...
server <- function(input, output, session) {
  ...
  observeEvent(input$run, {
    withProgress(message = 'Running CMiNet...', value = 0, {
      ...
      # if (!is.null(result$edge_list)) {
      #   edge_list <- as.matrix(result$edge_list)
      #   write.csv(edge_list, file.path("Network", "edge_list.csv"))
      # }
  ...
  ####################result tab  
  # Dynamically generate the UI for the Results tab
  output$resultsTab <- renderUI({
    ...
    tagList(
      ...
      # Conditionally include the Edge List download button
      if (edge_list_exists) {
        tags$div(
  ...
  #     if (file.exists(file_path)) {
  #       edge_list <- as.matrix(read.csv(file_path, row.names = 1))
  # 
  #       
  #       # Save the modified weighted network
  #       write.csv(edge_list, file, row.names = TRUE)
  #     } else {
  #       stop("The edge_list file does not exist.")
  #     }
  #   }
  # )

...
server <- function(input, output, session) {
  ...
  results <- eventReactive(input$run, {
    ...
    pearson_params <- if ("pearson" %in% selectedAlgorithms) {
      list(enabled = TRUE,params = list())
    } else {
      list(enabled = FALSE, params = list())
    }
    
    spearman_params <- if ("spearman" %in% selectedAlgorithms) {
      list(enabled = TRUE,params = list())
    } else {
      list(enabled = FALSE, params = list())
    }
    
    bicor_params <- if ("bicor" %in% selectedAlgorithms) {
      list(enabled = TRUE,params = list())
    } else {
      list(enabled = FALSE, params = list())
    }
    ...
    if (!is.null(result$weighted_network)) {
      weighted_network <- as.matrix(result$weighted_network)  # Ensure it's a matrix
